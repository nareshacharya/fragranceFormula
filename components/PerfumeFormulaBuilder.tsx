...
export default function PerfumeFormulaBuilder() {
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && over.id === 'formula-canvas') {
      const ingredient = active.data.current?.ingredient;
      if (ingredient && !isIngredientInFormula(ingredient.id)) {
        const newIngredient = {
          ...ingredient,
          concentration: ingredient.concentration || 5,
          quantity: ingredient.quantity || 10,
          cost: ingredient.cost || Math.round(Math.random() * 50 + 10)
        };
